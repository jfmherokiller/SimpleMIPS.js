...
export class INSTRUCTION_CLASSED {
    INST_UNSIGNED;
    INST_IMM_SHIFT;
    INST_REL_PC;
    INST_CAT;
...
// build translators
        ...
        this.INST_UNSIGNED = INST_UNSIGNED;
        this.INST_IMM_SHIFT = INST_IMM_SHIFT;
        this.INST_REL_PC = INST_REL_PC;
        this.INST_CAT = INST_CAT;
        this.INST_ALL = INST_ALL;
...
export class CPU {
    ...
    static MAX_PC = 0x10000000; // limit pc range in simulator
    ...
    static INSTRUCTION_CLASSED = new INSTRUCTION_CLASSED(CPU.instructionTable);

    constructor(mem,mode,callbacks?) {
        ...
        if (mode == CPU.SIM_MODE.FUNCTIONAL) {
            this.step = this._fStep;
            this.reset = this._fReset;
        } else {
            //this._initPipeline = this._initPipeline;
            //this.reset = this._aReset;
            //this.step = this._aStep;
        }
        this.reset();
    }
    ...
    _fStep(inDelaySlot) {
        ...
        switch (opcode) {
            case 0:
                switch (func) {
                    ...
                    case 43: // sltu rd, rs, rt
                        ...
                        break;
                    default:
                        exception |= CPU.EXCEPTION_CODE.INVALID_INST;
                }
                break;
            case 1:
                switch (rt) {
            ...
            case 33: // lh rt, offset(rs) sign extended
                ...
                if (tmp & 0x01) {
                    exception |= CPU.EXCEPTION_CODE.DATA_ALIGN;
                } else {
        ...
        if (nextPC & 0x3) {
            exception |= CPU.EXCEPTION_CODE.PC_ALIGN;
            nextPC += 4 - (nextPC & 0x3);
        }
        ...
        if (hasDelaySlot) {
            if (this.branchTarget) {
                exception |= CPU.EXCEPTION_CODE.BRANCH_IN_DELAY_SLOT;
            }
        ...
        if (this.pc > CPU.MAX_PC) {
            this.pc = CPU.MAX_PC;
            exception |= CPU.EXCEPTION_CODE.PC_LIMIT;
        }
...
class Pipeline extends CPU {
    ...
    stalls;
    constructor(mem,mode,callbacks) {
        super(mem,mode,callbacks);
        this.if_id ={pc: 0, inst: 0};
        this.id_ex = {
            // hardware
            aluOp: CPU.ALU_OP.NOP,		// alu function select
...
// saved writeback target
	...
	// saved writeback value
        ...
        switch (id_ex.aluOp) {
            ...
            // aluA - reg
            // aluB - reg, imm, sa
            case Pipeline.ALU_OP.ADD: // add
                tmp = ((aluA & 0x80000000) ? 0x100000000 + aluA : aluA) +
                    ((aluB & 0x80000000) ? 0x100000000 + aluB : aluB);
                break;
            case Pipeline.ALU_OP.SUB: // sub
                tmp = ((aluA & 0x80000000) ? 0x100000000 + aluA : aluA) -
                    ((aluB & 0x80000000) ? 0x100000000 + aluB : aluB);
                break;
            case Pipeline.ALU_OP.AND: // and
                tmp = aluA & aluB;
                break;
            case Pipeline.ALU_OP.OR: // or
                tmp = aluA | aluB;
                break;
            case Pipeline.ALU_OP.XOR: // xor
                tmp = aluA ^ aluB;
                break;
            case Pipeline.ALU_OP.NOR: // nor
                tmp = ~(aluA | aluB);
                break;
            case Pipeline.ALU_OP.SLL: // sll
                tmp = aluA << (aluB & 0x1f);
                break;
            case Pipeline.ALU_OP.SRL: // srl
                tmp = aluA >>> (aluB & 0x1f);
                break;
            case Pipeline.ALU_OP.SRA: // sra
                tmp = aluA >> (aluB & 0x1f);
                break;
            case Pipeline.ALU_OP.SLT: // less than cmp signed
                tmp = ((aluA | 0) < (aluB | 0));
                break;
            case Pipeline.ALU_OP.SLTU: // less than cmp unsigned
                tmp = (aluA < aluB);
                break;
            case Pipeline.ALU_OP.NOP:
            default: // pass through
                tmp = aluA;
        }
        if (id_ex.ovEn && (tmp / 2 >>> 31) ^ (tmp >>> 31)) {
            exception |= Pipeline.EXCEPTION_CODE.INT_OVERFLOW;
        ...
        switch (opCode) {
            case 0:
                switch (funcCode) {
                    ...
                    case 13: // break;
                        ...
                        exception |= Pipeline.EXCEPTION_CODE.BREAK;
                        break;
                    //case 16: // mfhi
                    //case 17: // mthi
                    //case 18: // mflo
                    ...
                    case 35: // subu rd, rs, rt
                        ...
                        break;
                    case 36: // and rd, rs, rt
                        newAluOp = Pipeline.ALU_OP.AND;
                        newOprASrc = rs;
                        newOprBSrc = rt;
                        newRegDst = rd;
                        break;
                    case 37: // or rd, rs, rt
                        newAluOp = Pipeline.ALU_OP.OR;
                        newOprASrc = rs;
                        newOprBSrc = rt;
                        newRegDst = rd;
                        break;
                    case 38: // xor rd, rs, rt
                        newAluOp = Pipeline.ALU_OP.XOR;
                        newOprASrc = rs;
                        newOprBSrc = rt;
                        newRegDst = rd;
                        break;
                    case 39: // nor rd, rs, rt
                        newAluOp = Pipeline.ALU_OP.NOR;
                        newOprASrc = rs;
                        newOprBSrc = rt;
                        newRegDst = rd;
                        break;
                    case 42: // slt rd, rs, rt
                        newAluOp = Pipeline.ALU_OP.SLT;
                        newOprASrc = rs;
                        newOprBSrc = rt;
                        newRegDst = rd;
                        break;
                    case 43: // sltu rd, rs, rt
                        newAluOp = Pipeline.ALU_OP.SLTU;
                        newOprASrc = rs;
                        newOprBSrc = rt;
                        newRegDst = rd;
                        break;
                    default:
                        exception |= Pipeline.EXCEPTION_CODE.INVALID_INST;
                }
                break;
            case 1:
                switch (rt) {
                    case 0: // bltz rs, offset
                        ...
                        branchCondSrcA = rs;
                        branchCond = Pipeline.BRANCH_COND.LTZ;
                        break;
                    case 1: // bgez rs, offset
                        prepareBranch = true;
                        branchTargetOffset = imms << 2;
                        branchCondSrcA = rs;
                        branchCond = Pipeline.BRANCH_COND.GTEZ;
                        break;
                    default:
                        exception |= Pipeline.EXCEPTION_CODE.INVALID_INST;
                }
            ...
            case 4: // beq rs, rt, offset
                ...
                branchTargetSrc = 32;
                branchTargetOffset = imms << 2;
                branchCond = Pipeline.BRANCH_COND.EQ;
                branchCondSrcA = rs;
                branchCondSrcB = rt;
                break;
            case 5: // bne rs, rt, offset
                ...
                branchTargetSrc = 32;
                branchTargetOffset = imms << 2;
                branchCond = Pipeline.BRANCH_COND.NEQ;
                branchCondSrcA = rs;
                branchCondSrcB = rt;
                break;
            case 6: // blez rs, offset
                ...
                branchTargetSrc = 32;
                branchTargetOffset = imms << 2;
                branchCond = Pipeline.BRANCH_COND.LTEZ;
                branchCondSrcA = rs;
                branchCondSrcB = 0;
                break;
            case 7: // bgtz rs, offset
        ...
        if (prepareBranch) {
            // confirm branch
            switch (branchCond) {
                case Pipeline.BRANCH_COND.LTZ:
                    confirmBranch = (branchCondValA & 0x80000000);
                    break;
                case Pipeline.BRANCH_COND.GTZ:
                    confirmBranch = (branchCondValA > 0 && branchCondValA < 0x80000000);
                    break;
                case Pipeline.BRANCH_COND.LTEZ:
                    confirmBranch = (branchCondValA & 0x80000000) || (branchCondValA == 0);
                    break;
                case Pipeline.BRANCH_COND.GTEZ:
                    confirmBranch = (branchCondValA < 0x80000000);
                    break;
                case Pipeline.BRANCH_COND.EQ:
                    confirmBranch = (branchCondValA == branchCondValB);
                    break;
                case Pipeline.BRANCH_COND.NEQ:
                    confirmBranch = (branchCondValA != branchCondValB);
                    break;
                case Pipeline.BRANCH_COND.N:
                default: // do nothing
                    confirmBranch = true;
            }
            // branch confirmed
            ...
            if (confirmBranch) {
                ...
                if (branchTarget & 0x03) {
                    exception |= Pipeline.EXCEPTION_CODE.PC_ALIGN;
                    confirmBranch = false;
                }
            }
        }
        ...
        if (stopPCUpdate) {
            debugInfo.stallFlag |= Pipeline.STALL_SET.ID;
            // write nop
            id_ex.aluOp = Pipeline.ALU_OP.NOP;
            id_ex.memOp = Pipeline.MEM_OP.NOP;
            id_ex.oprA = 0;
            id_ex.oprB = 0;
            id_ex.oprASrc = 0;
            id_ex.oprBSrc = 0;
        ...
        if (mem.busy) {
            ...
            if_id.inst = 0;
            stopPCUpdate = true;
            debugInfo.stallFlag |= Pipeline.STALL_SET.IF;
